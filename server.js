...
try {
  if (process.env.GOOGLE_CLOUD_CREDENTIALS) {
    ...
    console.log('✅ Google Vision API initialized (Railway credentials)');
  } else if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
    visionClient = new ImageAnnotatorClient({ keyFilename: process.env.GOOGLE_APPLICATION_CREDENTIALS });
    console.log('✅ Google Vision API initialized (local key file)');
  } else if (process.env.GOOGLE_SERVICE_ACCOUNT_JSON) {
    googleCredentials = JSON.parse(process.env.GOOGLE_SERVICE_ACCOUNT_JSON);
    visionClient = new ImageAnnotatorClient({ credentials: googleCredentials });
    console.log('✅ Google Vision API initialized (JSON env var)');
  } else {
    visionClient = new ImageAnnotatorClient();
    console.log('✅ Google Vision API initialized (default GCP credentials)');
  }
} catch (error) {
...
/**
 * Hugging Face Fallback – Using OwlViT (works reliably in Nov 2025)
 */
async function detectProductsWithHuggingFace(imageBuffer) {
  console.group('🤗 Hugging Face Detection (OwlViT)');
  ...
  console.log(`📡 Calling ${model} via router.huggingface.co`);
  ...
  const detections = rawDetections
    .filter(d => d.score > 0.25)  // OwlViT is very confident → lower threshold
    .map((item, i) => {
      const b = item.box || {};
  ...
  detections.forEach((d, i) => {
    console.log(`   ${i + 1}. ${d.name} (${(d.confidence * 100).toFixed(1)}%) → ${d.category}`);
  });
  console.groupEnd();
  return detections;
}
 ...
 */
app.post('/api/detect-products', async (req, res) => {
  console.group('📸 New Detection Request –', new Date().toISOString());
  ...
  try {
    ...
    const imageBuffer = Buffer.from(base64Data, 'base64');
    console.log(`🖼️ Image size: ${(imageBuffer.length / 1024).toFixed(1)} KB`);
    ...
    if (visionClient) {
      try {
        ...
        if (objects.length > 0) {
          ...
          usedProvider = 'google-vision';
          console.log(`✅ Google Vision found ${detections.length} objects`);
        }
      } catch (err) {
    ...
    // Final response
    console.log(`🎉 Detection complete – Provider: ${usedProvider}, Count: ${detections.length}`);
    console.groupEnd();
...
app.get('/', (req, res) => {
  res.json({
    message: 'Shop Mini Backend – Running Nov 2025 Fix',
